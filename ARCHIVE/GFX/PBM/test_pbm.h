...
namespace test_pbm {
	...
	int run() {
		INFO(__FUNCTION__);
		{
			...
			gfx::simple_bitmap_t* bmp = new gfx::simple_bitmap_t;
			gfx::init_simple_bitmap(bmp);
			gfx::simple_bitmap_t* p = gfx::new_simple_bitmap(2, 3, 1, 0, rgb, 1);
			...
			if (adapter == bios::HGC || adapter == bios::UNKNOWN) {
				...
				if (YESNO("graphics mode? ")) {
					...
					ANYKEY(""); 
					{
						hga::swap_buffers();
						hga::cls();						
						hga::vram_tile_block(10, 10, testbmp);
