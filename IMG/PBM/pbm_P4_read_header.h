 ...
 *
 */
#ifndef PBM_P4_READ_HEADER_H
#define PBM_P4_READ_HEADER_H
...
namespace pbm {
	...
	namespace P4 {
		...
		dos::error::codes read_pbm_header(std::string file_path, bitmap* img) {
			...
			uint16_t width, height, bytes, offset, size;
			...
			// 5. get header data
			while (f->peek() == '#') f->ignore(MAX_LINE_SIZE, '\n');	// skip any comments
			f->operator>>(width);				// read width
			f->operator>>(height);				// and height
			bytes = (int)width / 8;				// convert width to bytes
			bytes += (width & 7) == 0 ? 0 : 1;	// need an extra byte for width remainder < 8?
			bytes *= height;					// expected number bytes
			f->ignore(MAX_LINE_SIZE, '\n');		// data should start on next line
			offset = f->tellg();				// data starts here
			size = filesys::istream_size(f);
			if(offset + bytes != size) { // expected amount of data?
				f->close();
				delete f;
				return dos::error::INVALID_DATA;
			}
			// 6. populate the header
			img->header.file_path = file_path;
